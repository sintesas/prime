...
class GruposrelController extends AbstractActionController
{
    ...
    public function indexAction()
    {
		if($this->getRequest()->isPost()){
			...
			$not = new Gruposrel($this->dbAdapter);
			$auth = $this->auth;
			...
			$resultado=$not->addGruposrel($data,$id);



			//redirige a la pantalla de inicio del controlador
			if ($resultado==1){
				$this->flashMessenger()->addMessage("Grupo relacionado creado con exito");
				return $this->redirect()->toUrl($this->getRequest()->getBaseUrl().'/application/editargrupoinv/index/'.$id);
			}else
			{
				$this->flashMessenger()->addMessage("La creacion del grupo relacionado fallo");
				return $this->redirect()->toUrl($this->getRequest()->getBaseUrl().'/application/editargrupoinv/index/'.$id);
			}
		}
		else
