...
use Application\Editarredinv\ProyectosintredForm;
...
class ProyectosintredController extends AbstractActionController
{
	private $auth;
	public $dbAdapter;
    ...
    public function indexAction()
    {
		...
		$ProyectosintredForm = new ProyectosintredForm();
		$this->dbAdapter=$this->getServiceLocator()->get('db1');
		...
		if($id3 != 0){
			if($this->getRequest()->isPost()){
				$u = new Proyectosintred($this->dbAdapter);
                ...
                if($upload->isUploaded()){
                    ...
                    if($upload->isValid()){
                        ...
                        if($id2!=0){
                            $registro = $u->getProyectointredById($id2);
                            if($registro != null){
                                unlink("public/images/uploads/ri/proyectosi/ri_pi__".$id."_".$registro[0]["archivo"]);
                            }
                        }
                        $upload->setDestination('./public/images/uploads/ri/proyectosi/');
                        $upload->addFilter('File\Rename', array('target' => $upload->getDestination().DIRECTORY_SEPARATOR . "ri_pi__".$id."_".$file_name,'overwrite' => true));
    ...
    public function  asignarAction(){
		...
		$id3 = (int) $this->params()->fromRoute('id3',0);
		if($id3!=0){
			$resul = $u->updateProyectointred($id3, $id, "");
		}else{
    ...
    public function bajarAction()
    {
        ...
        $data = $u->getProyectointredById($id2);        
        foreach ($data as $d) {
            $fileName = $d["archivo"];
        }
        $response = new \Zend\Http\Response\Stream();   
        $response->setStream(fopen("public/images/uploads/ri/proyectosi/ri_pi__".$id."_".$filter->filter($fileName), 'r'));
        $response->setStatusCode(200);
        $headers = new \Zend\Http\Headers();
        $headers->addHeaderLine('Content-Type', 'whatever your content type is')
                ->addHeaderLine('Content-Disposition', 'attachment; filename="' . $filter->filter($fileName) . '"')
                ->addHeaderLine('Content-Length', filesize("public/images/uploads/ri/proyectosi/ri_pi__".$id."_".$filter->filter($fileName)));
