...
use Application\Convocatoria\AprobacionmForm;
...
class AprobacionmController extends AbstractActionController {
	private $auth;
	public $dbAdapter;
	...
	public function indexAction() {
		...
		if ($this->getRequest ()->isPost ()) {
			...
			$AprobacionmForm = new AprobacionmForm ();
			
			$id = (int) $this->params ()->fromRoute ('id', 0);
			$resultado = $not->updateAplicarAprobacion($id, $data);
			...
			$AprobacionmForm->add ( array (
				'name' => 'obervaciones_aprobacion',
				'attributes' => array (
						'type' => 'textarea',
						'maxlength' => "15000",
						'value' =>  $filter->filter($datAplicar ["obervaciones_aprobacion"])
				),
				'options' => array (
					'label' => 'Observaciones de la aprobación:', 
				),
			) );

			if($datAplicar ["fecha_verificacion"]==null){
				$datAplicar ["fecha_verificacion"] = date("Y-m-d");
			}
			...
			$AprobacionmForm->add(array(
				...
				'attributes' => array (
					'value' =>  $filter->filter($datAplicar ["fecha_verificacion"]),
					'disabled' => 'disabled'
				)
			));
			...
			$AprobacionmForm->add ( array (
				...
				'type' => 'Zend\Form\Element\Select',
				'attributes' => array (
					'value' =>  $filter->filter($datAplicar ["estado_aprobacion"])
				),
				'options' => array (
					'label' => 'Estado:',
					'value_options' => array(
			...
			$actasm = new Actasm ( $this->dbAdapter );
			...
			$informesm = new Informesm($this->dbAdapter);
			if (true) {					
					$view = new ViewModel ( array (
						'form' => $AprobacionmForm,
						'titulo' => "APROBACIÓN DE MONITORIA",
						...
						'informesmaplicacion' => $informesm->getInformeh($datAplicar["id_aplicar"]),
						'informesm' => $Avalcumpliconvo->getInformeActivoByIdConvo($datAplicar["id_convocatoria"]),
						'actasm' => $actasm->getArchivos($datAplicar["id_aplicar"])
					) );
					return $view;				
			} else {
				return $this->redirect ()->toUrl ( $this->getRequest ()->getBaseUrl () . '/application/mensajeadministrador/index' );
