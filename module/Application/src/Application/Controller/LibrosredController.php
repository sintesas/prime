...
use Application\Editarredinv\LibrosredForm;
...
class LibrosredController extends AbstractActionController
{
	private $auth;
	public $dbAdapter;
    ...
    public function indexAction()
    {
		...
		if($this->getRequest()->isPost()){
			...
			$this->dbAdapter=$this->getServiceLocator()->get('db1');
			$not = new Librored($this->dbAdapter);
			$auth = $this->auth;
			...
			$LibrosredForm = new LibrosredForm();
			//adiciona la noticia
			...
			if($id2!=0){
				$resultado=$not->updateLibrosred($id2, $data);
				...
				if($upload->isUploaded()){
                    ...
                    if($upload->isValid()){
                        ...
                        $resultado = $not->getLibroredById($id2);
                        if($resultado[0]["archivo"] != ""){
                            unlink("public/images/uploads/libros/ri_lib__".$id."_".$resultado[0]["archivo"]);
                        }
                        $upload->setDestination('./public/images/uploads/libros/');
                        $upload->addFilter('File\Rename', array('target' => $upload->getDestination().DIRECTORY_SEPARATOR . "ri_lib__".$id."_".$archi,'overwrite' => true));
                        $rtn = array('success' => null);
                        $rtn['success'] = $upload->receive();
                        $resultado = $not->updatearchivoLibrosred($id2,$archi);
	            ...
	            /* */
	                ...
	                if($upload->isValid()){
	                    $rtn = array('success' => null);
	                    $rtn['success'] = $upload->receive();
	                    $resultado=$not->addLibrored($data,$id, $archi);
	                    $this->flashMessenger()->addMessage("Libro agregado con éxito.");
	                }else{
	                	$resultado=$not->addLibrored($data,$id, "");
		...
		{
			$LibrosForm = new LibrosredForm();
			$this->dbAdapter=$this->getServiceLocator()->get('db1');
			$u = new Librored($this->dbAdapter);
	...
	public function bajarAction()
    {
        ...
        $data = $u->getLibroredById($id);        
        foreach ($data as $d) {
            $fileName = $d["archivo"];
        }
        $response = new \Zend\Http\Response\Stream();   
        $response->setStream(fopen("public/images/uploads/libros/ri_lib__".$id2."_".$filter->filter($fileName), 'r'));
        $response->setStatusCode(200);
        $headers = new \Zend\Http\Headers();
        $headers->addHeaderLine('Content-Type', 'whatever your content type is')
                ->addHeaderLine('Content-Disposition', 'attachment; filename="' . $filter->filter($fileName) . '"')
                ->addHeaderLine('Content-Length', filesize("public/images/uploads/libros/ri_lib__".$id2."_".$filter->filter($fileName)));
