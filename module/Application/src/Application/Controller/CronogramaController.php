...
use Application\Convocatoria\CronogramaForm;
...
class CronogramaController extends AbstractActionController
{
    ...
    public function indexAction()
    {
		...
		if($this->getRequest()->isPost()){
			...
			$this->dbAdapter=$this->getServiceLocator()->get('db1');
			$not = new Cronograma($this->dbAdapter);
			$auth = $this->auth;
			...
			$CronogramaForm = new CronogramaForm();
			//adiciona la noticia
			$id = (int) $this->params()->fromRoute('id',0);
			$resultado=$not->addCronograma($data, $id);			

			//redirige a la pantalla de inicio del controlador
			if ($resultado==1){
				$resul = $au->getAuditoriaid(session_id(), get_real_ip(), $identi->id_usuario);
                $evento = 'Creacion de cronograma : ' . $resultado . ' (mgc_cronograma)';
		...
		else
		{
			...
			$CronogramaForm = new CronogramaForm();
			$this->dbAdapter=$this->getServiceLocator()->get('db1');
			$u = new Cronograma($this->dbAdapter);
			...
			if(true){
			$id = (int) $this->params()->fromRoute('id',0);
			$view = new ViewModel(array('form'=>$CronogramaForm,
										'titulo'=>"Cronograma de la Convocatoria",
	...
	public function eliminarAction(){
		...
		$resul = $au->getAuditoriaid(session_id(), get_real_ip(), $identi->id_usuario);
		$evento = 'Eliminación de cronograma : ' . $id . ' (mgc_cronograma)';
		$ad->addAuditoriadet($evento, $resul);
