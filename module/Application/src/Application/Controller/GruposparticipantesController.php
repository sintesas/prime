...
use Application\Convocatoria\GruposparticipantesForm;
...
class GruposparticipantesController extends AbstractActionController
{
	private $auth;
	public $dbAdapter;
    ...
    public function indexAction()
    {
		...
		if($this->getRequest()->isPost()){
			$this->dbAdapter=$this->getServiceLocator()->get('db1');
			$u = new Gruposparticipantes($this->dbAdapter);
			...
			$r='';
			$GruposparticipantesForm = new GruposparticipantesForm();
			$view = new ViewModel(array('form'=>$GruposparticipantesForm,'titulo'=>"Asignar Grupo Participante",'grupo'=>$id,'datos'=>$us->filtroGrupos($data),'menu'=>$dat["id_rol"]));
			return $view;
		}else
		{
	    $this->dbAdapter=$this->getServiceLocator()->get('db1');
        ...
        return $this->redirect()->toUrl($this->getRequest()->getBaseUrl().'/application/login/index');
			...
			$GruposparticipantesForm->add(array(
			'name' => 'roles',
...
$id = (int) $this->params()->fromRoute('id',0);
		$a=null;
		if(true){
		$view = new ViewModel(array('form'=> $GruposparticipantesForm,
...
$this->flashMessenger()->addMessage("El Usuario ya tiene un rol asignado");
			return $this->redirect()->toUrl($this->getRequest()->getBaseUrl().'/application/aplicar/index/'.$id2);
}
	}
